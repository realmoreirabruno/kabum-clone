...
@Composable
fun HomeScreen(navController: NavController, produtoViewModel: ProductViewModel = koinViewModel<ProductViewModel>()) {
    val produtos by produtoViewModel.produtos.collectAsState()

    LaunchedEffect(Unit) {
        produtoViewModel.loadProdutos()
    }

    val destaques = produtos.take(3)
    val recomendados = produtos.drop(3)
            ...
            LazyRow(
                ...
                items(destaques) { produto ->
                    ProdutoCardHorizontal(produto = produto, onClick = {
                        navController.navigate("produto/${produto.productId}")
                    })
                }
            }
        }
